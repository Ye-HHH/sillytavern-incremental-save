...
// Image proxy: intercept Image.src setter to route external images through the caching proxy
...
export async function saveChat({ chatName, withMetadata, mesId, force = false, chatData = undefined } = {}) {
    ...
    try {
        ...
        if (canAppend || canHeaderOnly) {
            ...
            console.log(`[Save-Size] INCREMENTAL ${debugAction}: ${body.length} bytes`);
        ...
        console.log(`[Save-Size] FULL save: ${fullBody.length} bytes (${trimmedChat.length} messages)`);
        const saveChatRequest = await compressRequest({
            method: 'POST',
            cache: 'no-cache',
            headers: getRequestHeaders(),
...
export async function getChat() {
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            // Initialize incremental save tracking on chat load
            lastSavedChatName = characters[this_chid].chat;
            lastSavedChatLength = chat.length;
            lastSavedChatHash = computeChatHash(chat, chat.length);
            console.log(`[Save-Size] Tracking initialized: ${lastSavedChatName} (${lastSavedChatLength} messages)`);
        } else {
